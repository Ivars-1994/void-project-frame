...
if(isset($settings['link']) && !empty($settings['link']) && count($settings['link'])): ?>
    <ul class="pxl-link pxl-link-l1 <?php echo esc_attr( $settings['hover_style'].' '.$settings['color_style'].' '.$settings['display_style'].' '.$settings['pxl_animate'].' '.$settings['divider'] ); ?>">
        <?php
            foreach ($settings['link'] as $key => $link):
                $display_style = isset($link['display_style']) ? $link['display_style'] : '';
                ...
                ?>
                <li class="<?php echo esc_attr($settings['pxl_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['pxl_animate_delay']); ?>ms">
