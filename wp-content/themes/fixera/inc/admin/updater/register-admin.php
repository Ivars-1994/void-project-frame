<?php
/**
 * The Fixera_Register initiate the theme engine
 */

if( !defined( 'ABSPATH' ) ) 
	exit; // Exit if accessed directly

class Fixera_Register {
	...
	public function __construct( $config = array(), $strings = array() ) {
		$config = wp_parse_args( $config, array(
			...
			'theme_slug'     => fixera()->get_slug(),
			'theme_name'     => fixera()->get_name(),
			'version'        => '',
	...
	function updater() {
		...
		if ( !class_exists( 'Fixera_Updater' ) ) {
	...
	function admin_error() {
		echo '<div class="error"><p>' . sprintf( wp_kses_post( esc_html__( 'The %s theme needs to be registered. %sRegister Now%s', 'fixera' ) ), fixera()->get_name(), '<a href="' . admin_url( 'admin.php?page=pxlart') . '">' , '</a>' ) . '</p></div>';
	}
	
	function admin_notice() {
		echo '<div class="notice"><p>'.esc_html__( 'Purchase code is invalid. Need a license for activation', 'fixera' ).'</p></div>';
	}
	...
	function messages($merlin = false){
		...
		if ( ! $purchase_code ){
			...
			<div class="pxl-dsb-box-head-inner">
				<h6><?php echo esc_html__( 'Register License', 'fixera' ) ?></h6>
			</div>
			<?php 
			$this->form();
			?>
			<div class="pxl-dsb-box-foot">
				<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'Can’t find your purchase code?', 'fixera' ); ?></a>
			</div>
			<?php 
		}else{  
		...
		if ( false === $license_data->success ) {
			switch ( $license_data->error ) {
				...
				case 'item_name_mismatch':
					$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'fixera' ), $this->theme_name );
				break;
				case 'license_exists':
			...
			<div class="pxl-dsb-confirmation fail">
				<h6><?php echo esc_html__( 'Active false', 'fixera' ) ?></h6>
				<p><?php echo wp_kses_post( $message ) ?> <a href="<?php echo esc_url($pxl_server_info['docs_url']) ?>" target="_blank"><?php echo esc_html__( 'our help center', 'fixera' ) ?></a> or <a href="<?php echo esc_url($pxl_server_info['support_url']) ?>" target="_blank"><?php echo esc_html__( 'submit a ticket', 'fixera' ) ?></a></p>
			</div>
			<?php $this->form(); ?>
			<div class="pxl-dsb-box-foot">
				<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'Can’t find your purchase code?', 'fixera' ); ?></a>
			...
			if ( 'valid' === $license_data->license ) {
				...
				<div class="pxl-dsb-box-head"> 
					<div class="pxl-dsb-confirmation success">
						<h6><?php echo esc_html__( 'Thanks for the verification!', 'fixera' ) ?></h6>
						<p><?php echo esc_html__( 'You can now enjoy and build great websites. Looking for help? Visit', 'fixera' ) ?> <a href="<?php echo esc_url($pxl_server_info['support_url']) ?>" target="_blank"><?php echo esc_html__( 'submit a ticket', 'fixera' ) ?></a>.</p>
					</div> 
					...
					<div class="pxl-dsb-deactive">
						<form method="POST" action="<?php echo admin_url( 'admin.php?page=pxlart' )?>">
							...
							<button class="btn button" type="submit"><?php esc_html_e( 'Remove Purchase Code', 'fixera' ) ?></button>
						</form>
					</div> 
				</div> 
				<?php 
		        ...
		        $message = esc_html__( 'Response return null.', 'fixera' ); ?>
		        <div class="pxl-dsb-confirmation fail">
		          <h6><?php echo esc_html__( 'Active false', 'fixera' ) ?></h6>
		          <p><?php echo wp_kses_post( $message ) ?> <a href="<?php echo esc_url($pxl_server_info['docs_url']) ?>" target="_blank"><?php echo esc_html__( 'our help center', 'fixera' ) ?></a> or <a href="<?php echo esc_url($pxl_server_info['support_url']) ?>" target="_blank"><?php echo esc_html__( 'submit a ticket', 'fixera' ) ?></a></p>
		        </div>
	...
	function form() {
		...
		<form action="options.php" method="post" class="pxl-dsb-register-form">
			...
			<input id="<?php echo esc_attr($this->theme_slug)?>_purchase_code" name="<?php echo esc_attr($this->theme_slug)?>_purchase_code" type="text" value="<?php echo esc_attr( $license ); ?>" placeholder="<?php esc_attr_e( 'Enter your purchase code', 'fixera' ); ?>">
			<input type="submit" class="res-purchase-code" value="<?php esc_attr_e( 'Register your purchase code', 'fixera' ) ?>">
		</form>
		<?php
	}
