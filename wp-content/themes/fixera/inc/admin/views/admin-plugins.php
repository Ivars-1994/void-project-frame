...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'fixera');
...
$merlin_setup = get_option( 'merlin_' . fixera()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'fixera');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'fixera');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'fixera');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'fixera' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'fixera' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'fixera' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Fixera_Admin_Plugins;
