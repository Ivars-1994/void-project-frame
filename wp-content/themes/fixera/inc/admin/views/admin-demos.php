<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		if ( 'valid' != get_option( fixera()->get_slug().'_purchase_code_status', false ) && !$dev_mode ) :
			
			echo '<div class="error"><p>' .
					sprintf( wp_kses_post( esc_html__( 'The %s theme needs to be registered. %sRegister Now%s', 'fixera' ) ), fixera()->get_name(), '<a href="' . admin_url( 'admin.php?page=pxlart') . '">' , '</a>' ) . '</p></div>';
		elseif( !empty($plugin_requires) && sizeof($plugin_requires) >= 1 ):

			echo '<div class="error"><p>';
		   echo sprintf( wp_kses_post( esc_html__( 'Make sure to activate required plugins prior to import a demo.', 'fixera' ) ), fixera()->get_name(), '<a class="nt-atpli" href="' . admin_url( 'admin.php?page=pxlart-plugins') . '">' , '</a>' ) . '</p>';
		...
		<header class="pxl-dsb-header">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Import a Demo', 'fixera' ); ?></h4>
				<p><?php esc_html_e( 'Choose a pre-built website for starting a quick design process.', 'fixera' ) ?></p>
			</div>
			<div class="pxl-msg pxl-dsb-notice">
				<p><span><?php esc_html_e( 'Note:', 'fixera' ); ?></span> <?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'fixera' ) ?></p>
			</div>
		</header>
							...
							<figure>
								...
								<div class="pxl-btn-group">
									<a href="#" id="import-id" data-import-id="<?php echo esc_attr( $i ); ?>" data-demo-id="<?php echo esc_attr( $id ); ?>" class="pxl-btn pxl-popup-import">
										<span><?php esc_html_e( 'Import Demo', 'fixera' ); ?></span>
									</a><br/>
									<a target="_blank" href="<?php echo esc_url( $demo['preview'] ); ?>" class="pxl-preview-btn">
										<span><?php esc_html_e( 'Preview Demo', 'fixera' ); ?></span>
		...
		<script type="text/template" id="tmpl-demo-import-modules">
			<div id="pxl-progress-popup" class="pxl-imp-popup-wrap is-active">
				<div class="pxl-imp-progress">
					<h6><?php esc_html_e( 'Importing...', 'fixera' ); ?></h6>
					<div id="pxl-progress" class="importing"><?php esc_html_e( 'Working', 'fixera' )?> <span>.</span><span>.</span><span>.</span></div>
					<div class="pxl-progressbar">
						<div class="pxl-progressbar-inner" style="width: 0%">
							<span id="pxl-loader" class="pxl-progressbar-percentage"><?php esc_html_e( '0%', 'fixera' ); ?></span>
						</div>
					</div>
				</div>
			</div>
			...
			<div id="pxl-popup" class="pxl-imp-popup-wrap is-active">
				<div class="pxl-imp-popup-inner">
					...
					<div class="pxl-imp-popup-content">
						<h4 style="text-align:center; margin-bottom: 30px;"><?php esc_html_e( 'Select all or a few', 'fixera' ); ?></h4>
						<div class="pxl-row">
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Media Attachments', 'fixera' ); ?></span>
								</span>
							</div>
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Content', 'fixera' ); ?></span>
								</span>
							</div>
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Theme Options', 'fixera' ) ?></span>
								</span>
							</div>
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Widgets', 'fixera' ); ?></span>
								</span>
							</div>
							...
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Revslider', 'fixera' ); ?></span>
								</span>
							</div>
 							<?php endif; ?>
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Settings', 'fixera' ) ?></span>
								</span>
							</div>
						</div>
							...
							<div class="pxl-col pxl-col-12">
								<div class="pxl-imp-skip-posts">
									<span class="pxl-imp-opt-skip-posts" style="margin-bottom: 0; padding-left: 15px;">
										...
										<span><?php esc_html_e( 'Skip the posts exist. By default, all current content will be deleted.', 'fixera' ); ?></span>
										</label>
									</span>
								</div>
								<div class="pxl-imp-crop">
									<span class="pxl-imp-opt-crop" style="margin-bottom: 0; padding-left: 15px;">
										...
										<label for="pxl-imp-crop-img">
											<span><?php esc_html_e( 'Crop Image after import finish?', 'fixera' ); ?></span>
										</label>
									</span>
								</div>
