<?php
if (!class_exists('Fixera_Comment')) {
     
    class Fixera_Comment
    {
        ...
        public function comment_extra_control ( $comment ) {
            ...
            <p>
                <label for="phone"><?php esc_html_e( 'Phone','fixera' ); ?></label>
            ...
            ?>
            <p>
                <label for="subject"><?php esc_html_e( 'Subject','fixera' ); ?></label>
            ...
            <p>
                <label for="rating"><?php esc_html_e( 'Rating: ','fixera' ); ?></label>
        ...
        public function comment_rating_fields ($args =[]) {
            ...
            if($this->show_rating == '1' && is_singular('post')){
                $rating .= '<div class="pxl-comment-form-rating pxl-comment-form-fields-wrap row gx-20 align-items-center '.esc_attr($args['class']).'">';
                    $rating .= '<div  class="comment-form-field col-auto">'. esc_html__('Your Rating','fixera').'<span class="required">*</span></div>';
        ...
        function wc_comment_rating_fields($args =[]){
            ...
            if (wc_review_ratings_enabled() && is_singular('product') ) {
                $rating .= '<div class="pxl-comment-form-rating pxl-comment-form-fields-wrap row gutters-12 align-items-center '.esc_attr($args['class']).'">';
                    $rating .= '<div class="comment-form-field col-12 col-sm-auto">' . esc_html__( 'Your rating of this product', 'fixera' ) . ( wc_review_ratings_required() ? '&nbsp;<span class="required">*</span>' : '' ) . '</div>';
                    $rating .= '<div class="comment-form-field comments-rating col">';
                        $rating .= '<select name="rating" id="rating" required>
                            <option value="">' . esc_html__( 'Rate&hellip;', 'fixera' ) . '</option>
                            <option value="5">' . esc_html__( 'Perfect', 'fixera' ) . '</option>
                            <option value="4">' . esc_html__( 'Good', 'fixera' ) . '</option>
                            <option value="3">' . esc_html__( 'Average', 'fixera' ) . '</option>
                            <option value="2">' . esc_html__( 'Not that bad', 'fixera' ) . '</option>
                            <option value="1">' . esc_html__( 'Very poor', 'fixera' ) . '</option>
                        </select>';
                    $rating .= '</div>';
                $rating .= '</div>';
            }
        ...
        public function comment_rating_require_rating( $commentdata ) {
            ...
            wp_die( esc_html__( 'Error: You did not add a rating. Hit the Back button on your Web browser and resubmit your comment with a rating.','fixera' ) );
            return $commentdata;
        }
                        ...
                        <div class="comment-content col">
                            <div class="row gx-10 justify-content-between">
                                <div class="col-auto">
                                    ...
                                    <div class="comment-date empty-none"><?php echo get_comment_date('F j, Y') ?></div>
                                </div>
                                <div class="col-auto">
                                <?php
                                    comment_reply_link( array_merge( $args, array(
                                        ...
                                        'reply_text' => esc_html__('Reply', 'fixera')
                                    ) ) ); 
                                ?>
                                </div>
                            </div>
        ...
        public function comment_form_args($args = []){
            ...
            $pxl_comment_fields = array(
                ...
                'cancel_reply_link'    => esc_attr__( 'Cancel Reply', 'fixera'),
                'title_reply_before'   => '<h4 id="reply-title" class="comment-reply-title">',
                'title_reply_after'    => '</h4>',
                'id_submit'            => 'submit',
                'class_submit'         => 'btn pxl-btn-submit',
                ...
                'comment_notes_before' => '<span class="lbl text-heading require-note">'.esc_html__( 'Your email address will not be published *', 'fixera' ).'</span>',
                'comment_field'        =>  '',
            );
            ...
            //open
            $pxl_fields['open'] .= '<div class="pxl-comment-form-fields-wrap row gx-30">';
            // author
            $pxl_fields['author'] = '<div class="comment-form-field comment-form-author col-lg-4 col-md-4 col-sm-12">'.
                '<label>'.esc_attr__('Your Name*', 'fixera').'</label><input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) .
                '" size="30" placeholder="'.esc_attr__('Name', 'fixera').'"/></div>';

            // email 
            $pxl_fields['email'] = '<div class="comment-form-field comment-form-email col-lg-4 col-md-4 col-sm-12">'.
                '<label>'.esc_attr__('Your Email*', 'fixera').'</label><input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) .
                '" size="30" placeholder="'.esc_attr__('Email', 'fixera').'"/></div>';

            //phone
            if($this->show_phone == '1'){
                $pxl_fields['phone'] = '<div class="comment-form-field comment-form-phone col-lg-4 col-md-4 col-sm-12">'.
                '<label>'.esc_attr__('Your Phone*', 'fixera').'</label><input id="phone" name="phone" type="text" size="30" placeholder="'.esc_attr__('Phone', 'fixera').'"/></div>';
            }
            ...
            if($this->show_subject == '1'){
                ...
                '" size="30" placeholder="'.esc_attr__('Subject', 'fixera').'"/></div>';
            } 
            ...
            if ( has_action( 'set_comment_cookies', 'wp_set_comment_cookies' ) && get_option( 'show_comments_cookies_opt_in' ) ) {
                ...
                $pxl_fields['cookies'] = sprintf(
                    ...
                    sprintf(
                        ...
                        esc_html__( 'Save my details in this browser for the next time I comment.', 'fixera' )
                    )
                ); 
            }
            ...
            $pxl_comment_fields['comment_field'] .= '<div class="pxl-comment-form-fields-wrap pxl-comment-form-fields-message row"><div class="comment-form-field comment-form-comment col-12"><label>'.esc_attr__('Message', 'fixera').'</label><textarea id="comment-msg" name="comment" cols="45" rows="8" placeholder="'.esc_attr__('Comment', 'fixera').'" aria-required="true">' .'</textarea></div></div>';
        ...
        function comment_product_form_args($args = []){
            ...
            $pxl_comment_fields = array(
                'id_form'              => 'commentform',
                'title_reply'          => esc_attr__( 'Leave a Review', 'fixera'),
                'title_reply_to'       => esc_attr__( 'Leave a Review To ', 'fixera') . '%s',
                'cancel_reply_link'    => esc_attr__( 'Cancel Reply', 'fixera'),
                'title_reply_before'   => '<h4 id="reply-title" class="comment-reply-title">',
                'title_reply_after'    => '</h4>',
                'id_submit'            => 'submit',
            ...
            $pxl_fields['phone'] = '<div class="comment-form-field comment-form-phone col-lg-4 col-md-4 col-sm-12">'.
                '<input id="phone" name="phone" type="text" size="30" placeholder="'.esc_attr__('Phone', 'fixera').'"/></div>';      
            ...
            $pxl_comment_fields['comment_field'] .= '<div class="pxl-comment-form-fields-wrap pxl-comment-form-fields-message row"><div class="comment-form-field comment-form-comment col-12"><textarea id="comment-msg" name="comment" cols="45" rows="8" placeholder="'.esc_attr__('Your review', 'fixera').'" aria-required="true">' .'</textarea></div></div>';
            return $pxl_comment_fields;
